...
- **Dynatrace Agent** (`.github/agents/dynatrace/`) — Connects to Dynatrace Davis AI, runs DQL queries, detects anomalies. Uses skills in `.github/skills/dynatrace/`.
- **DataPower Agent** (`.github/agents/datapower/`) — Analyzes DataPower reports and generates professional insights. Uses skills in `.github/skills/datapower/`. Operates independently with no dependencies on Dynatrace.
- **Structure Monitor** (`.github/agents/structure-monitor/`) — Automatically detects structural changes in the project and keeps all README.md files and Mermaid diagrams in sync. Configured via `.github/customizations/auto-sync.md`. Also manages automatic Git commits and pushes.

Two additional agents are present:
- **ai-team-dev** (`.github/agents/ai-team-dev.agent.md`) — A three-role dev team (Nova/Frontend, Sage/Backend, Milo/Visual) for feature implementation. Reads `PROJECT_BRIEF.md` before starting and writes progress to `docs/sprint-N/progress.md`.
- **Atlassian Requirements to Jira** (`.github/agents/atlassian-requirements-to-jira.agent.md`) — Transforms requirements documents into Jira epics and user stories via the Atlassian MCP Server. Requires explicit user approval before any create/update operation; enforces limits of 20 epics / 50 stories per batch.
...
The `atlassian-requirements-to-jira` agent requires the Atlassian MCP Server to be installed and configured in VS Code before use. Without it, the agent cannot connect to Jira.
